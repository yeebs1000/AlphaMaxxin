...
def test_live_ohlcv_prefers_moomoo_kline(monkeypatch):
    from app.brokers import moomoo_client
    monkeypatch.setattr(moomoo_client, "MOOMOO_AVAILABLE", True)
    rows = [{"close": 10.0, "high": 11.0, "low": 9.0, "volume": 100.0},
            {"close": 12.0, "high": 13.0, "low": 11.0, "volume": 200.0}]
    ...
    assert bars == {"closes": [10.0, 12.0], "highs": [11.0, 13.0],
                    "lows": [9.0, 11.0], "volumes": [100.0, 200.0]}
